...
class Text {
    // varcode function constant...
    const CODE128A_START_BASE = 103; 
    const CODE128B_START_BASE = 104;
    const CODE128C_START_BASE = 105;
    const STOP = 106;
    
    public static function highlight($text, $words) {
      preg_match_all('~\w+~', $words, $m);
      $re = '~(' . implode('|', $m[0]) . ')~i';
      return preg_replace($re, '<SPAN class="bg-highlight"><b>$0</b></span>', $text);    
    }

    
    public static function barcode($code, $showText=false, $fileName=null, $type='png', $height=30, $thin=1, $thick=2, $fSize=1) {
      $bc = new BarCode($type, $height, $thin, $thick, $fSize);
      return $bc->build($code, $showText, $fileName);
    }
    
    // this functions was copied from: 
    // https://github.com/anigenero/PHP-Barcode-Generator/blob/master/barcode.php
    public static function barcode_128($code , $density = 1) {
      ...
      $encoding	=	array($code128_bar_codes[self::CODE128B_START_BASE]);
      ...
      //Return the image
      return base64_encode($output_img);
